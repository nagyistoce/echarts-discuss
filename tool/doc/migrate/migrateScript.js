...
define(function (require) {
    ...
    var $ = require('jquery');
    var schemaHelper = require('./schema/schemaHelper');
    var util = require('./schema/util');
    var Set = require('./schema/Set');
        ...
        for (var name in originData) {
            if (originData.hasOwnProperty(name)) {
                schema.definitions[name] = {
                    type: 'Object',
                    properties: util.clone(originData[name])
                };
            }
        }
    ...
    function setDefault(schema) {
        schemaHelper.travelSchema(schema, function (o) {
            ...
            if (defaultValueHTML === 'null') {
                ...
                util.assert(type.contains('boolean'));
                o[DF] = defaultValueHTML === 'true' ? true : false;
            }
            else if (defaultValueHTML.indexOf('<pre>') === 0) {
                util.assert(type.contains('Object') || type.contains('Array'));
                util.assert(defaultValueHTML.indexOf('</pre>') === defaultValueHTML.length - '</pre>'.length);
                var str = defaultValueHTML.replace('<pre>', '').replace('</pre>', '');
                o[DF] = util.parseToObject(str);
            }
            else if (defaultValueHTML.indexOf('{') === 0 && defaultValueHTML !== '{...}') {
                util.assert(type.contains('Object'));
                util.assert(defaultValueHTML.indexOf('}') === defaultValueHTML.length - 1);
                o[DF] = util.parseToObject(defaultValueHTML);
            }
            else if (defaultValueHTML.indexOf('[') === 0) {
                util.assert(type.contains('Array'));
                util.assert(defaultValueHTML.indexOf(']') === defaultValueHTML.length - 1);
                o[DF] = util.parseToObject(defaultValueHTML);
            }
            else if (type.contains('string') || type.contains('color') || type.contains('Function')) {
                o[DF] = o.defaultValueHTML;
            }
            else {
